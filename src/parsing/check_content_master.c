...
void	process_token(t_map *map, char *line_read, char tmp[1000], int i)
{
	...
	if (token_nbr == 1)
		map->north_texture = check_content_no(tmp, i, line_read);
	else if (token_nbr == 2)
		map->south_texture = check_content_so(tmp, i, line_read);
	else if (token_nbr == 3)
		map->west_texture = check_content_we(tmp, i, line_read);
	else if (token_nbr == 4)
		map->east_texture = check_content_ea(tmp, i, line_read);
	else if (token_nbr == 5)
	{
		map->fc_taken = true;
		map->is_cc = false;
...
int	check_content_master(t_map *map)
{
	...
	while (line_read)
	{
		...
		if (map->north_texture && map->south_texture && map->west_texture
			&& map->east_texture && map->fc_taken && map->cc_taken)
			break ;
		line_read = read_and_clean_line(line_read, map);
	}
	parse_map(map);
...
//final check of map_char for t_map *map
bool	verif_map(t_map *map, int i, int nbr_player)
{
	...
	if (ext_is_all_wall(map) == 0)
		return (true);
	if (!map->copy_map)
		map->copy_map = copy_map(map->map_char);
	x = map->player_x;
	y = map->player_y;
	flood_fill(x, y, map);
	if (map->is_closed == false)
		return (true);
	return (false);
}
